<script lang="ts">
	...
	const { form, enhance, reset, errors, constraints } = superForm(data.form, {
		...
		onUpdated: ({ form }) => {},
		onError({ result }) {
			console.log('Email verification token -->Superform On Error --> ', result.error.message);
		}
	});
</script>
...
<section class="flex flex-col w-screen min-h-[75vh] px-4">
	<div class="md:max-w-xl mx-auto my-auto card px-4">
		<header class="card-header">
			<h2 class="h2 font-bold">Email verification</h2>
		...
		<form method="post" use:enhance>
			...
			<button type="submit" class="btn my-2 w-full variant-filled">Resend verification link</button>
		</form>
		<footer class="card-footer">
			{#if success}
				<p class="text-center font-semibold">Your email verification link was sent to your inbox</p>
			{/if}
